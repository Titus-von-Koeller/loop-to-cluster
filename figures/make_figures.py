...
def fig_param_breakdown():
    ...
    rows = [("MLP  (3 gated matrices)", P_MLP), ("Embedding  (tied)", P_EMBED),
            ("Attention  (GQA, 3 KV heads)", P_ATTN), ("RMSNorm scales", P_NORM)]
    ...
    for i, v in enumerate(vals):
        ...
        ax.text(v + P_TOTAL * 0.012, i, lab, va="center", fontsize=10,
                color=INK_2)
    ...
    ax.grid(False)
    despine(ax, keep=())
    title(ax, "Where 134,515,008 parameters actually live",
          "SmolLM2-135M. The MLP is nearly three fifths of the model.")
    note(fig, "Measured: sum(p.numel()) over the constructed model, deduplicated for the tied embedding.")
...
def fig_textbook_error():
    ...
    title(ax, "The textbook parameter formula is wrong for modern models",
...
def fig_memory_buckets():
    ...
    for i, n in enumerate(names):
        ax.barh([0], [per], left=left, color=BLUE_ORDINAL[i], height=0.5,
                edgecolor=SURFACE, linewidth=2.5)
        ax.text(left + per / 2, 0, f"{n}\n4 B/param", ha="center", va="center",
                fontsize=9.5, color="white" if i >= 2 else INK, fontweight="bold")
        left += per
    ax.text(left / 2, 0.44, f"16 bytes per parameter  ·  {left:,.0f} MiB  ≈  {left/1024:.2f} GiB",
    ...
    ax.grid(False)
    despine(ax, keep=())
    title(ax, "The four model-state buckets", "Fixed by the model and optimizer. Activations sit on top of this.")
    note(fig, "Measured: 134,515,008 params x 4 B. Mixed precision does not move any of these four.")
...
def fig_optimizer_cost():
    ...
    states = [0, 4, 8, 2]
    base = 8
    ax.bar(names, [base] * 4, color=BLUE_ORDINAL[1], width=0.5,
           edgecolor=SURFACE, linewidth=2, label="params + grads (8 B)")
    ax.bar(names, states, bottom=[base] * 4, color=GOLD, width=0.5,
           edgecolor=SURFACE, linewidth=2, label="optimizer state")
    for i, s in enumerate(states):
        ax.text(i, base + s + 0.35, f"{base + s} B", ha="center",
                fontsize=11, color=INK, fontweight="bold")
    ax.set_ylabel("bytes per parameter")
    ax.set_ylim(0, 19)
    ax.legend(loc="upper left", ncol=2, bbox_to_anchor=(0, 1.02))
    despine(ax, keep=("bottom",))
    title(ax, "The optimizer is the cheapest memory lever you have",
...
def fig_float_formats():
    ...
    fmts = [("fp32", 1, 8, 23, "32 bits"), ("fp16", 1, 5, 10, "16 bits"),
            ("bf16", 1, 8, 7, "16 bits")]
    for row, (name, s, e, m, tot) in enumerate(fmts):
        y = 2 - row
        x = 0
        for width, color in ((s, INK_2), (e, BLUE), (m, GOLD)):
            ax.add_patch(Rectangle((x, y - 0.29), width, 0.58, facecolor=color,
                                   edgecolor=SURFACE, linewidth=2.5))
            if width > 1:
                ax.text(x + width / 2, y, str(width), ha="center", va="center",
                        fontsize=11, color="white", fontweight="bold")
            x += width
        ax.text(-0.5, y, name, ha="right", va="center", fontsize=12.5,
                fontweight="bold", color=INK)
        ax.text(x + 0.6, y, tot, ha="left", va="center", fontsize=10, color=MUTED)

    ax.annotate("", xy=(9, 2.62), xytext=(1, 2.62),
                arrowprops=dict(arrowstyle="<->", color=BLUE, lw=1.7))
    ax.text(5, 2.80, "same exponent width  →  same range", ha="center",
            fontsize=10, color=BLUE, fontweight="bold")
    for xv in (1, 9):
        ax.plot([xv, xv], [-0.05, 2.56], color=BLUE, lw=1.1, ls=(0, (3, 3)), zorder=0)

    for i, (color, lab) in enumerate(((INK_2, "sign"), (BLUE, "exponent → range"),
                                      (GOLD, "mantissa → precision"))):
        lx = i * 12.0
        ax.add_patch(Rectangle((lx, -0.78), 1.1, 0.36, facecolor=color,
                               edgecolor="none"))
    ...
    ax.axis("off")
    ax.text(-6, -1.18, "bf16 keeps fp32's exponent and pays for it out of the mantissa. "
            "fp16 does the opposite — and falls off both ends of the range.",
            fontsize=9.8, color=INK_2, ha="left")
    fig.suptitle("Two different bets on how to spend 16 bits", x=0.052, y=1.02,
...
def fig_fp16_underflow():
    ...
    for xv, lab, ha in ((fp16_sub, "smallest subnormal\n6.0e-8", "right"),
                        (fp16_normal, "smallest normal\n6.1e-5", "left")):
        ax.annotate(lab, xy=(xv, top), xytext=(xv * (0.55 if ha == "right" else 1.8),
                                               top * 1.14),
                    fontsize=9.5, color=ROSE, fontweight="bold", ha=ha,
                    va="bottom", annotation_clip=False,
                    arrowprops=dict(arrowstyle="-", color=ROSE, lw=1.1))
    ax.text(3e-10, top * 0.55, "flushed to zero\nunder fp16",
            fontsize=10.5, color=ROSE, ha="center", fontweight="bold")
    ax.annotate("", xy=(3e-4, top * 0.13), xytext=(4e-8, top * 0.13),
                arrowprops=dict(arrowstyle="-|>", color=GOLD, lw=2.4,
                                mutation_scale=18))
    ax.text(1.1e-6, top * 0.175,
            "loss scaling multiplies every gradient by S",
            fontsize=9.8, color="#7d5f10", ha="center", fontweight="bold")
    ax.set_xscale("log")
    ...
    despine(ax, keep=("bottom",))
    title(ax, "Why fp16 needs loss scaling and bf16 does not",
          "bf16's smallest normal is ~1.2e-38 — far off this chart to the left. Nothing here underflows.")
    note(fig, "Schematic: the distribution is illustrative, not measured, so read the shape and not the area. "
...
def fig_loss_anatomy():
    ...
    spike = healthy.copy()
    spike[150:168] += np.array([0, 1.4, 3.2, 4.1, 3.4, 2.5, 1.8, 1.2, 0.8,
                                0.5, 0.33, 0.2, 0.13, 0.08, 0.05, 0.03, 0.02, 0.01])
    ...
    for ax, (name, yv, color, flag, _) in zip(axes, panels, strict=True):
        ...
        if name == "Divergence":
            ...
            ax.text(x[261] + 0.03, tail[261], "NaN", fontsize=11, color=ROSE,
                    fontweight="bold", va="center")
        elif flag:
            ax.text(0.5, lnV + 0.75, flag, fontsize=10, color=ROSE,
    ...
    fig.text(0.008, 0.965, "What the loss curve is telling you",
             fontsize=14, fontweight="bold", color=INK, ha="left", va="top")
    fig.text(0.008, 0.885, "Every run starts from the same place: a randomly initialized "
             "language model must begin at ln(vocab_size).",
             fontsize=10.3, color=INK_2, ha="left", va="top")
    fig.text(0.008, 0.045, "Schematic: curve shapes, not measurements. "
...
def fig_memory_over_step():
    ...
    ax.annotate("peak: everything stored,\nnothing yet released",
...
def fig_zero_stages():
    ...
    for i in range(4):
        ...
        ax.text(i, tot + 0.35, f"{tot:.0f} B" if tot == int(tot) else f"{tot:.1f} B",
                ha="center", fontsize=11, color=INK, fontweight="bold")
    ax.set_ylabel("bytes per parameter, per GPU")
    ax.set_ylim(0, 19)
    ax.legend(loc="upper right")
    despine(ax, keep=("bottom",))
    title(ax, "What each ZeRO stage actually shards",
          "Shown for 4 ranks. The optimizer is the biggest bucket, which is why stage 1 goes after it first.")
    note(fig, "Arithmetic, for N=4 ranks. Communication volume rises as the sharding deepens; that cost is not shown.")
...
def fig_ragged_shard():
        ...
        for b in range(per + 1):
            for s in range(bs):
                ...
                if idx < n_samples:
                    ...
                    ax.add_patch(Rectangle((x, y - 0.34), 0.92, 0.68,
                                           facecolor=color, edgecolor=SURFACE, linewidth=2))
                    ax.text(x + 0.46, y, str(idx), ha="center", va="center",
                            fontsize=8.5, color="white", fontweight="bold")
                else:
                    ax.add_patch(Rectangle((x, y - 0.34), 0.92, 0.68,
                                           facecolor="none", edgecolor=ROSE,
                                           linewidth=1.6, linestyle=(0, (3, 2))))
                    ax.text(x + 0.46, y, "—", ha="center", va="center",
    ...
    ax.grid(False)
    despine(ax, keep=())
    ax.text(0, -0.62, "26 samples · 4 ranks · batch size 2  →  three clean rounds, then a ragged one.\n"
            ...
            fontsize=9.5, color=INK_2, ha="left", va="top", transform=ax.get_yaxis_transform(which="grid"))
    title(ax, "The last batch never divides evenly",
          "Every rank must take the same number of steps, or the collective deadlocks.")
    save(fig, f"{OUT}/10_ragged_shard.png")


for f in (fig_param_breakdown, fig_textbook_error, fig_memory_buckets,
          fig_optimizer_cost, fig_float_formats, fig_fp16_underflow,
          fig_loss_anatomy, fig_memory_over_step, fig_zero_stages,
          fig_ragged_shard):
    f()
print("done")
