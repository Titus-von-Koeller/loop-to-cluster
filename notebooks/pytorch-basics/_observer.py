...
_PHI = np.linspace(-45.0, 45.0, 13)
_W1 = np.geomspace(0.05, 12.0, 8)
_W2 = np.geomspace(0.05, 12.0, 8)
...
def _p_correct_grid(cols, du, gj, size):
...
def _log_posterior(records, chunk=40_000):
    cols = _grid_columns()
    du, gj, size, ok = _trial_features(records)
    n_cells = len(cols[0])
    logp = np.zeros(n_cells, dtype=np.float64)
    for lo in range(0, n_cells, chunk):
        sl = slice(lo, lo + chunk)
        p = _p_correct_grid([c[sl] for c in cols], du, gj, size)
...
def _posterior_mean(post, axis_name, log_space=False):
    vals, m = _marginal(post, axis_name)
    if log_space:
        return float(np.exp((m * np.log(vals)).sum()))
    return float((m * vals).sum())


def _threshold_de(post, cols, direction, g_j, p_target=0.75, size=104.0):
...
def fit(log_path, cache=True, force=False):
    ...
    logp, cols = _log_posterior(records)
    post = np.exp(logp)
    post /= post.sum()

    phi_vals, phi_m = _marginal(post, "phi")
    payload = {
        "model": MODEL_VERSION,
        "n": len(records),
        "phi_deg_mean": float((phi_m * phi_vals).sum()),
        "phi_deg_sd": float(np.sqrt((phi_m * phi_vals**2).sum() - ((phi_m * phi_vals).sum()) ** 2)),
        "w1_mean": _posterior_mean(post, "w1", log_space=True),
        "w2_mean": _posterior_mean(post, "w2", log_space=True),
        "beta_mean": _posterior_mean(post, "beta"),
        "lapse_mean": _posterior_mean(post, "lam"),
        "tau0_mean": _posterior_mean(post, "tau0", log_space=True),
        "gL_mean": _posterior_mean(post, "gL"),
        "gamma_mean": _posterior_mean(post, "gamma"),
    }
    # Directional thresholds at the reference (mid) ground and at the two measured pages.
    payload["de_dir_at_mid"] = {name: _threshold_de(post, cols, v, 0.5) for name, v in _DIRECTIONS.items()}
    for label, g in (("day", "#fdf0ed"), ("night", "#1c1e26")):
        g_j = float(hex_to_ucs(g)[0, 0] / 100.0)
        payload[f"de_dir_{label}"] = {name: _threshold_de(post, cols, v, g_j) for name, v in _DIRECTIONS.items()}
    ...
    payload["marginals"] = {
        name: {
            "values": [float(v) for v in _marginal(post, name)[0]],
            "p": [float(v) for v in _marginal(post, name)[1]],
        }
        for name in _AXES
    }
    if cache:
